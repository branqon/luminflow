...
var NOTE_GAP = { crystal: 30, pluck: 80, sub: 200 };
var SPLAT_SCALE = {
...
function getZoneFromPosition(x, y, rect) {
  ...
  if (ratio < 0.3) {
    ...
    zone = "pluck";
    ringDepth = (ratio - 0.3) / 0.35;
  } else {
    zone = "sub";
    ringDepth = Math.min((ratio - 0.65) / 0.35, 1);
  }
...
function MusicalWavesV2() {
  ...
  const startDrone = useCallback(
    (source = "latched") => {
      ...
      const pos = boundingRef.current ? getZoneFromPosition(mouseRef.current.x, mouseRef.current.y, boundingRef.current) : { zone: "pluck", angle: 0, ringDepth: 0.5 };
      const zone = pos.zone;
  ...
  const triggerNote = useCallback(
    (x, y) => {
      ...
      const now2 = Tone.now();
      if ((now2 - lastNoteTimeRef.current[zone]) * 1e3 < NOTE_GAP[zone]) return;
      lastNoteTimeRef.current[zone] = now2;
      setCurrentZone(zone);
      const scaleSize = SCALES[currentScale].intervals.length;
      ...
      if (zone === "crystal") {
        velocity = 0.15 + ringDepth * 0.25 + clamp(dragSpeed * 2e-3, 0, 0.08);
        duration = 0.12;
      } else if (zone === "pluck") {
        velocity = 0.12 + ringDepth * 0.23;
        duration = 0.1 + (1 - clamp(dragSpeed / 80, 0, 1)) * 0.4;
      } else {
        velocity = 0.1 + ringDepth * 0.18;
        duration = 0.3;
      }
      try {
  ...
  const runFlowStep = useCallback(() => {
    ...
    let spiralRadius = 0.05 + t * 3e-3 * flowRef.current.direction;
  ...
  const tick = useCallback(
    (time) => {
      ...
      if (cursorRef.current) {
        const curZone = boundingRef.current ? getZoneFromPosition(mouse.sx, mouse.sy, boundingRef.current).zone : "pluck";
        const halfSize = pointerDownRef.current ? { crystal: 10, pluck: 12, sub: 15 }[curZone] : { crystal: 7, pluck: 8, sub: 10 }[curZone];
      ...
      if (!pointerDownRef.current) {
        ...
        if (sim && rect) {
          ...
          sim.addSplat(
            ...
            1 - mouseRef.current.y / rect.height,
            (mouseRef.current.x - mouseRef.current.lx) * 5e-4,
            -(mouseRef.current.y - mouseRef.current.ly) * 5e-4,
            color,
            radius
          );
        }
    ...
    "div",
    {
      ...
      children: [
              ...
              introPhase !== "playing" && /* @__PURE__ */ jsxs(
                "div",
                {
                  ...
                  children: [
                    ...
                    (introPhase === "ripple" || introPhase === "invite") && /* @__PURE__ */ jsx("div", { className: "mw-intro-invite", children: /* @__PURE__ */ jsx("p", { children: "Touch anywhere to begin" }) })
                  ]
                }
              )
            ]
        ...
        /* @__PURE__ */ jsxs(
          "div",
          {
            className: `mw-watermark ${activeEdge === "bottom" || activeEdge === "left" ? "bright" : ""}`,
